var _ = require("lodash");
var __Counters = {};

module.exports = {
  all    : getAll,
  create : create,
  inc    : applyTo("count", inc),
  dec    : applyTo("count", dec),
  delete : del
};

function genId() {
  return (+new Date() + ~~(Math.random * 999999)).toString(36);
}

function getAll() { return _.map(__Counters, _.identity); }

function create(title, count) {
  var id = genId();
  __Counters[id] = {id: id, title: title, count: count};
  console.log(__Counters[id]);
  return __Counters[id];
}

function del(id) {
  delete __Counters[id];
  return id;
}

function applyTo(key, fn) {
  return function(id) {
    __Counters[id][key] = fn(__Counters[id][key]);
    return getAll();
  }
}

function inc(n) { return parseInt(n) + 1; }
function dec(n) { return parseInt(n) - 1; }
